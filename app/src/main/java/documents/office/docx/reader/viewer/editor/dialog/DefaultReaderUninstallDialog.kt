...
import documents.office.docx.reader.viewer.editor.databinding.DefaultReaderUninstallDialogBinding
...
class DefaultReaderUninstallDialog : DialogFragment() {
    ...
    var defaultWordViewer: ResolveInfo? = null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (TemporaryStorage.isLoadAds) {
            ...
            val appIcon = defaultWordViewer?.loadIcon(pm)
            val packageName = defaultWordViewer?.activityInfo?.packageName

            binding.fileName.text = defaultWordViewer?.loadLabel(pm).toString()
